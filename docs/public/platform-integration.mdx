...
const DEFAULT_PORT = 37777; // Override with CLAUDE_MEMU_WORKER_PORT
```
...
```bash
CLAUDE_MEMU_MODEL=claude-sonnet-4-5          # Model for observations/summaries
CLAUDE_MEMU_CONTEXT_OBSERVATIONS=50          # Observations injected at SessionStart
CLAUDE_MEMU_WORKER_PORT=37777                # Worker service port
CLAUDE_MEMU_PYTHON_VERSION=3.13              # Python version for chroma-mcp
```
...
```json Response
{
  ...
  "database": {
    "path": "~/.claude-memu/claude-memu.db",
    "size": 1048576,
    "observations": 500,
    "sessions": 50,
    "summaries": 25
...
describe('Worker Integration', () => {
  ...
  beforeAll(async () => {
    ...
    workerProcess = spawn('node', ['dist/worker-service.js'], {
      env: { ...process.env, CLAUDE_MEMU_WORKER_PORT: '37778' }
    });
...
<CardGroup cols={2}>
  ...
  <Card title="GitHub" icon="github" href="https://github.com/minhlucvan/claude-memu">
