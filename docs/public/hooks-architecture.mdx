...
Claude-Memu uses lifecycle hook scripts across 5 hook events: Setup, SessionStart, UserPromptSubmit, PostToolUse, and Stop. All hooks (except Setup) use the `bun-runner.js` wrapper to invoke `worker-service.cjs` subcommands.

### Pre-Hook: Smart Install (Part of SessionStart)

**Purpose:** Intelligently manage dependencies before the worker starts.

**Note:** This is the first hook in the SessionStart sequence, running before the worker-service hooks.
...
**Configuration:** (first hook in SessionStart sequence)
```json
{
  "hooks": {
    "SessionStart": [{
...
- ✅ Configurable context limit via `CLAUDE_MEMU_CONTEXT_LIMIT`
...
|-------|--------|----------|---------|-----------------|
| **Setup** | Plugin install | No | 120s | setup.sh |
...
| **SessionStart (user-message)** | Before session | No | 60s | stderr (log only) |
| **UserPromptSubmit (worker-start)** | Before processing | No | 60s | stderr (log only) |
| **UserPromptSubmit (session-init)** | Before processing | No | 60s | stdout → context |
| **PostToolUse (worker-start)** | After tool | No | 60s | stderr (log only) |
| **PostToolUse (observation)** | After tool | No | 120s | Transcript only |
| **Stop (worker-start)** | On stop | No | 60s | stderr (log only) |
| **Stop (summarize)** | On stop | No | 120s | Database |
...
<AccordionGroup>
  ...
  <Accordion title="Observations not captured">
    ...
    **Debugging:**
    1. Check database: `sqlite3 ~/.claude-memu/data/ "SELECT * FROM observation_queue"`
...
**Privacy guarantees:**
- All data stored locally in `~/.claude-memu/data/`
