...
|---------|--------|---------|-------------|
| `CLAUDE_MEMU_PROVIDER` | `claude`, `gemini`, `openrouter` | `claude` | AI provider for observation extraction |
| `CLAUDE_MEMU_OPENROUTER_API_KEY` | string | — | Your OpenRouter API key |
| `CLAUDE_MEMU_OPENROUTER_MODEL` | string | `xiaomi/mimo-v2-flash:free` | Model identifier (see list above) |
| `CLAUDE_MEMU_OPENROUTER_MAX_CONTEXT_MESSAGES` | number | `20` | Max messages in conversation history |
| `CLAUDE_MEMU_OPENROUTER_MAX_TOKENS` | number | `100000` | Token budget safety limit |
| `CLAUDE_MEMU_OPENROUTER_SITE_URL` | string | — | Optional: URL for analytics attribution |
| `CLAUDE_MEMU_OPENROUTER_APP_NAME` | string | `claude-mem` | Optional: App name for analytics |
...
```json
{
  "CLAUDE_MEMU_PROVIDER": "openrouter",
  "CLAUDE_MEMU_OPENROUTER_API_KEY": "sk-or-v1-your-key-here",
  "CLAUDE_MEMU_OPENROUTER_MODEL": "xiaomi/mimo-v2-flash:free"
}
```
...
Either:
- Set `CLAUDE_MEMU_OPENROUTER_API_KEY` in `~/.claude-memu/settings.json`, or
...
- Reduce `CLAUDE_MEMU_OPENROUTER_MAX_CONTEXT_MESSAGES`
- Reduce `CLAUDE_MEMU_OPENROUTER_MAX_TOKENS`
...
HTTP-Referer: https://github.com/minhlucvan/claude-memu
X-Title: claude-mem
