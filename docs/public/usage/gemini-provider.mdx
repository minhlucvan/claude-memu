...
|---------|--------|---------|-------------|
| `CLAUDE_MEMU_PROVIDER` | `claude`, `gemini` | `claude` | AI provider for observation extraction |
| `CLAUDE_MEMU_GEMINI_API_KEY` | string | — | Your Gemini API key |
| `CLAUDE_MEMU_GEMINI_MODEL` | `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash` | `gemini-2.5-flash-lite` | Gemini model to use |
| `CLAUDE_MEMU_GEMINI_BILLING_ENABLED` | `true`, `false` | `false` | Skip rate limiting if billing is enabled on Google Cloud |
...
```json
{
  "CLAUDE_MEMU_PROVIDER": "gemini",
  "CLAUDE_MEMU_GEMINI_API_KEY": "your-api-key-here",
  "CLAUDE_MEMU_GEMINI_MODEL": "gemini-2.5-flash-lite",
  "CLAUDE_MEMU_GEMINI_BILLING_ENABLED": "true"
}
```
...
Either:
- Set `CLAUDE_MEMU_GEMINI_API_KEY` in `~/.claude-memu/settings.json`, or
