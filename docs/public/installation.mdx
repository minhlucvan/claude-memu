...
git clone https://github.com/minhlucvan/claude-memu.git
cd claude-mem
...
Data is stored in `~/.claude-memu/`:
- Database: `~/.claude-memu/claude-memu.db`
- PID file: `~/.claude-memu/.worker.pid`
- Port file: `~/.claude-memu/.worker.port`
- Logs: `~/.claude-memu/logs/worker-YYYY-MM-DD.log`
- Settings: `~/.claude-memu/settings.json`

Override with environment variable:
```bash
export CLAUDE_MEMU_DATA_DIR=/custom/path
```
...
See [CHANGELOG](https://github.com/minhlucvan/claude-memu/blob/main/CHANGELOG.md) for complete version history.
