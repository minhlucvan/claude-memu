...
1. Clear install cache:
   ```bash
   rm ~/.claude/plugins/marketplaces/minhlucvan/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.claude/plugins/marketplaces/minhlucvan/.install-version
   cat ~/.claude/plugins/marketplaces/minhlucvan/package.json | grep version
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.claude-memu/claude-memu.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.claude-memu/claude-memu.db "SELECT COUNT(*) FROM observations;"
   ```
...
3. **View stuck messages directly**:
   ```bash
   sqlite3 ~/.claude-memu/claude-memu.db "
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.claude-memu/claude-memu.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.claude-memu/claude-memu.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.claude-memu/claude-memu.db "VACUUM;"
   ```
...
3. Check package.json exists:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/minhlucvan/package.json
   ```
...
2. Check for stale locks:
   ```bash
   lsof ~/.claude-memu/claude-memu.db
   ```
...
2. Backup database:
   ```bash
   cp ~/.claude-memu/claude-memu.db ~/.claude-memu/claude-memu.db.backup
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.claude-memu/claude-memu.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.claude-memu/claude-memu.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.claude-memu/claude-memu.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
3. Verify installation:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/minhlucvan/
   ```
...
**Solution**: Check `~/.claude-memu/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/minhlucvan/claude-memu/issues)
