...
description: "Complete guide to the memory agent hook lifecycle for platform implementers"
---

# Hook Lifecycle

Claude-Memu implements a **hook system** with 5 registered hooks (Setup, SessionStart, UserPromptSubmit, PostToolUse, Stop) that captures development work across Claude Code sessions. All hooks use the `bun-runner.js` wrapper to invoke `worker-service.cjs` subcommands. This document provides a complete technical reference for developers implementing this pattern on other platforms.
...
graph TB
    subgraph EXT["Extension Process (runs in IDE)"]
        ...
        subgraph HOOK_HANDLERS["Lifecycle Hooks"]
            H0[Setup<br/>setup.sh]
            H1[SessionStart<br/>smart-install + worker + context + user-message]
            H2[UserPromptSubmit<br/>worker + session-init]
            H3[PostToolUse<br/>worker + observation]
            H4[Stop<br/>worker + summarize]
        end
    ...
    subgraph WORKER["Worker Process (separate Node.js)"]
        ...
        AGENT -->|Event-Driven| STORE[UnifiedStore]
        STORE --> LOCAL[(LocalStore<br/>JSON files)]
        STORE --> MEMU[(MemuStore<br/>memU API)]
    end
...
| Stage | Hook Event | Worker Subcommand | Trigger | Purpose |
|-------|-----------|-------------------|---------|---------|
| **0. Setup** | `Setup` | `setup.sh` | Plugin installation | Install dependencies |
| **1. SessionStart** | `SessionStart` | `context` + `user-message` | User opens Claude Code | Smart-install, start worker, inject prior context |
| **2. UserPromptSubmit** | `UserPromptSubmit` | `session-init` | User submits a prompt | Start worker, create/init session |
| **3. PostToolUse** | `PostToolUse` | `observation` | Claude uses any tool | Start worker, queue observation |
| **4. Stop** | `Stop` | `summarize` | User stops asking questions | Start worker, generate session summary |

## Hook Configuration

Hooks are configured in `plugin/hooks/hooks.json`. All lifecycle hooks use the `bun-runner.js` wrapper to invoke `worker-service.cjs` subcommands:
...
**Note:** There is no `SessionEnd` hook. Session cleanup is handled internally by the worker service.
...
1. `smart-install.js` - Ensures dependencies are installed (300s timeout)
2. `bun-runner.js worker-service.cjs start` - Starts the worker service (60s timeout)
3. `bun-runner.js worker-service.cjs hook claude-code context` - Fetches and injects prior session context (60s timeout)
4. `bun-runner.js worker-service.cjs hook claude-code user-message` - Captures user message (60s timeout)
...
> **Note:** There is no `SessionEnd` hook registered in hooks.json. Session cleanup is handled internally by the worker service.
...
- [ ] 5 hook types: Setup, SessionStart (4 hooks), UserPromptSubmit (2 hooks), PostToolUse (2 hooks), Stop (2 hooks)
...
### Hook Implementation
- [ ] worker-service hook context: `GET /api/context/inject` (with health check)
- [ ] worker-service hook session-init: createSession, saveUserPrompt, `POST /sessions/{id}/init`
- [ ] worker-service hook observation: Skip low-value tools, `POST /api/sessions/observations`
- [ ] worker-service hook summarize: Parse transcript, `POST /api/sessions/summarize`
