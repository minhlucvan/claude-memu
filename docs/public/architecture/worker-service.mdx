...
- **Port**: Fixed port 37777 (configurable via `CLAUDE_MEMU_WORKER_PORT`)
- **Location**: `src/services/worker-service.ts`
- **Built Output**: `plugin/scripts/worker-service.cjs`
- **Model**: Configurable via `CLAUDE_MEMU_MODEL` environment variable (default: sonnet)
...
- PID file tracking at `~/.claude-memu/worker.pid`
...
```bash
export CLAUDE_MEMU_MODEL=sonnet
```
...
- **Default**: Port 37777
- **Override**: Set `CLAUDE_MEMU_WORKER_PORT` environment variable
- **Port**: Configured via `CLAUDE_MEMU_WORKER_PORT` setting or environment variable
