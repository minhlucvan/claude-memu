...
- **Logs**: `~/.claude-memu/logs/worker-YYYY-MM-DD.log`
- **PID File**: `~/.claude-memu/.worker.pid`
- **Port File**: `~/.claude-memu/.worker.port` (new)
...
- Database file: `~/.claude-memu/claude-memu.db` (same location)
...
`ensureWorkerRunning()` checks if `~/.claude-memu/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.claude-memu/.pm2-migrated` exists
</Step>
...
**Location**: `~/.claude-memu/.pm2-migrated`
...
5. **Marker creation**: `~/.claude-memu/.pm2-migrated` with timestamp
...
New: ~/.claude-memu/logs/worker-YYYY-MM-DD.log
```
...
New: ~/.claude-memu/.worker.pid
     ~/.claude-memu/.worker.port
     ~/.claude-memu/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.claude-memu/claude-memu.db
```
...
```
~/.claude-memu/
├── claude-memu.db          # Database (unchanged)
...
```
~/.claude-memu/
├── claude-memu.db          # Database (same file)
├── chroma/                # Vector embeddings (unchanged)
├── logs/
...
**Manual Resolution**:
```bash
rm ~/.claude-memu/.worker.pid
rm ~/.claude-memu/.worker.port
...
| `Invalid port X` | Port validation failed | Update `CLAUDE_MEMU_WORKER_PORT` in settings |
...
cat ~/.claude-memu/.pm2-migrated  # Should exist
