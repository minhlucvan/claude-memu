...
const INSTALLED_PATH = path.join(os.homedir(), '.claude', 'plugins', 'marketplaces', 'minhlucvan');
const CACHE_BASE_PATH = path.join(os.homedir(), '.claude', 'plugins', 'cache', 'minhlucvan', 'claude-memu');
...
try {
  execSync(
    'rsync -av --delete --exclude=.git --exclude=/.mcp.json ./ ~/.claude/plugins/marketplaces/minhlucvan/',
    { stdio: 'inherit' }
  );

  console.log('Running npm install in marketplace...');
  execSync(
    'cd ~/.claude/plugins/marketplaces/minhlucvan/ && npm install',
    { stdio: 'inherit' }
  );
